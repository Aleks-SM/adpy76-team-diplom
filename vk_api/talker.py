...
class Talker:
    ...
    async def plain_text_without_buttons(
        self,
        text: str,
    ):
        await self.bot.api.messages.send(
            user_id=self.user_id, message=text, random_id=0
        )
    ...
    async def plain_text_with_main_buttons(self, text: str):
        ...
        await self.bot.api.messages.send(
            user_id=self.user_id, message=text, keyboard=keyboard, random_id=0
        )
        return keyboard
    ...
    async def vk_search_user(self, vk_user_data: VkUserSearch):
        await self.bot.api.messages.send(
            ...
            message=f"{vk_user_data.name}\n{vk_user_data.profile_link}",
            random_id=0,
        )
        photo_uploader = PhotoMessageUploader(self.bot.api)
        async with aiohttp.ClientSession() as session:
            await self.bot.api.messages.send(
                user_id=self.user_id, message="Фото из профиля", random_id=0
            )
            for photo_url in vk_user_data.photos:
                ...
                await self.bot.api.messages.send(
                    user_id=self.user_id, attachment=attachment_photo, random_id=0
                )
            if vk_user_data.related_photos:
                await self.bot.api.messages.send(
                    user_id=self.user_id, message="Отмечен на фото", random_id=0
                )
    ...
    async def menu_buttons(self):
        keyboard = (
            Keyboard(one_time=True)
            .add(
                Text("Поиск", payload={"cmd": MenuButtonEnum.SEARCH.value.__str__()}),
        ...
        await self.bot.api.messages.send(
            user_id=self.user_id, keyboard=keyboard, random_id=0, message="💞💞💞"
        )
        return keyboard
    ...
    async def gender_request_with_buttons(self, text: str):
        keyboard = (
            Keyboard(one_time=True)
            .add(
                Text("Мужской", payload={"cmd": GenderEnum.MALE.value.__str__()}),
                color=KeyboardButtonColor.NEGATIVE,
            )
            .add(
                Text("Женский", payload={"cmd": GenderEnum.FEMALE.value.__str__()}),
                color=KeyboardButtonColor.POSITIVE,
            )
            .row()
            .add(
                Text(
                    "Не имеет значения", payload={"cmd": GenderEnum.ANY.value.__str__()}
                )
            )
            .get_json()
        )
